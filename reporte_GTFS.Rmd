---
title: "Reporte Estancia Profesional 'Primera Actividad' "
author: "Rafael Hernández Bravo"
date: "2025-05-19"
output:
...
### Lugar de la estancia: Centro de Ciencias de la Complejidad, Laboratorio.


### 1. Introducción

En este reporte se detalla el proceso de obtención, carga, análisis y visualización de datos GTFS (General Transit Feed Specification). El objetivo principal es explorar un conjunto de datos de transporte público, identificar sus características clave y generar visualizaciones y artefactos útiles, como mapas de rutas, paradas y archivos shapefile.

Los datos GTFS son un estándar abierto utilizado por agencias de transporte público para publicar sus horarios, información geográfica de rutas y paradas, y otra información operativa. Analizar estos datos permite entender la estructura y funcionamiento de los sistemas de transporte.

# Carga de librerías esenciales para el análisis de datos GTFS y manipulación espacial.

```{r}

library(tidytransit)
library(terra) # No se usa explícitamente en el script gtfs_frecuencias.R pero estaba listada
library(sp)    # No se usa explícitamente en el script gtfs_frecuencias.R pero estaba listada
library(sf)
library(dplyr)

# RUTA AL DIRECTORIO DE TRABAJO.
# Es crucial que esta ruta sea correcta para que el script funcione.
PATH = "C:/git_ep/GTFS" # MODIFICAR SI ES NECESARIO

# ESTABLECER DIRECTORIO DE TRABAJO.
setwd(PATH)

```
### 2. Carga, visualización y transformación de datos GTFS.
...
##### 2.1 grafico de barras del top 20 de paradas con mayor cantidad de registros de "trips" (Histograma)

## 3. Preparación de Datos y Generación de Shapefile de Rutas con Información de Paradas y Viajes

Generación de Shapefile a partir de Datos Combinados:

 1. Consolidación de Datos: Unión tipo outterjoin de Catálogo de Paradas, Frecuencias, Horarios y Viajes. (Describe los primeros merge)
 2. Conversión de Geometrías de Rutas (Shapes) a Formato sf. (Describe shapes_as_sf)
 3. Vinculación de Geometrías de Rutas con Datos Consolidados. (Describe el merge con shapes_sf)
 4. Exportación del Conjunto de Datos Geográficos a Archivo Shapefile (.shp). (Describe st_write)
...
# se tiene que agregar el .shp para escribir el archivo

if(F){st_write(df_shapes_stop_trips, "C:/proyecto_ep/stops_trips.shp")}
```

## 4. Conclusiones 

Este análisis de frecuencia absoluta proporciona una primera aproximación a la importancia relativa de las paradas dentro de la red de transporte público. Las paradas con mayor frecuencia son puntos clave en el sistema, ya sea por ser intercambiadores importantes, estar en zonas de alta demanda o ser puntos de inicio/fin de múltiples rutas.